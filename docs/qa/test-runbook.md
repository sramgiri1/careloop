...
| Reminder -> snooze -> escalation -> deep link | `backend:reminders`, `ios:reminders`; backend verifies escalation fanout, legacy null-receiver escalation safety, non-PII delivery summaries, and concurrent-worker claim-once behavior for reminder/escalation sends, and iOS verifies organizer Activity escalation timeline copy |
...
- Focused escalation fanout/timeline validation can be run with backend tests `escalation fanout logs a sanitized timeline summary without blocking on disabled alerts`, `pending reminders are claimed once when scheduler workers overlap`, and `sent reminders are escalated once when scheduler workers overlap`, plus Xcode UI test `test_organizerActivityShowsEscalationTimelineEntry`.
